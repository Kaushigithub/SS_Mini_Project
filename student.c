...
{
    //open faculty file 
   int fd1=open("faculty.txt",O_RDONLY);
    
   //open course file
   int fd2=open("course.txt",O_RDWR);

   //open enrolled courses file
   int fd3=open("course_enroll.txt",O_RDWR,0777);
   int flag=0;
   struct CourseDetail course;
   struct EnrolledCourse course_enroll;  
 while(read(fd2,&course,sizeof(struct CourseDetail)))
 {
    if(course.id==course_id)
    {
      while(read(fd3,&course_enroll,sizeof(course_enroll)))
      {
        if(course_enroll.student_id==student_id && course_enroll.course_id==course_id)
        {
           ...
           else if(course.no_seats>0)
           {
            ...
            course_copy.fac_id=course.fac_id;
            strcpy(course_copy.course_name,course.course_name);
            strcpy(course_copy.dept,course.dept);
            course_copy.no_seats=course.no_seats-1;
            course_copy.max_seats=course.max_seats;
            course_copy.course_credits=course.course_credits;
            write(fd2,&course_copy,sizeof(struct CourseDetail));
            send(client_socket,"Successfully Enrolled\n",sizeof("Successfully Enrolled"),0);
            lseek(fd3,-(sizeof(struct EnrolledCourse)),SEEK_CUR);
            struct EnrolledCourse course_en;
            course_en.student_id=student_id;
            course_en.course_id=course_id;
            course_en.enrolled=1;
            strcpy(course_en.course_name,course.course_name);
            course_en.fac_id=course.fac_id;     
            write(fd3,&course_en,sizeof(struct EnrolledCourse));           
           }
           else
           {
            send(client_socket,"Sorry No Seats Available Now...Try Later Sometime.\n",sizeof("Sorry No Seats Available...Try Later Sometime."),0);
           }
        }
        
      } 

      if(course.no_seats>0 && flag==0)
      {
        lseek(fd2,-(sizeof(struct CourseDetail)),SEEK_CUR);
        struct CourseDetail course_copy;
        course_copy.id=course_id;
     ...
     {
        send(client_socket,"Sorry No Seats Available...Try Later Sometime.\n",sizeof("Sorry No Seats Available...Try Later Sometime"),0);
     }
    }      
 }
  close(fd1);
...
void denroll_student(int client_socket,int student_id,int course_id)
{
    ...
    int fd2=open("course.txt",O_RDWR);
    int fd3=open("course_enroll.txt",O_RDWR);
    if(fd==-1||fd2==-1||fd3==-1)
    {
        perror("ERROR:");
    }
    struct CourseDetail course;
    struct EnrolledCourse course_enr;
    while((read(fd2,&course,sizeof(struct CourseDetail))))
    {        
     if(course.id==course_id)
     {
        while((read(fd3,&course_enr,sizeof(struct EnrolledCourse))))
        {
          if(course_enr.student_id==student_id&&course_enr.course_id==course_id)
          {
            if(course.no_seats<course.max_seats)
            {   
             lseek(fd2,-(sizeof(struct CourseDetail)),SEEK_CUR);
             struct CourseDetail course_copy;
             course_copy.id=course_id;
...
{
    int fd=open("course_enroll.txt",O_RDWR);
    if(fd==-1)
    {
        perror("ERROR:");
    }
    struct EnrolledCourse course;
    int ack=1;
    // printf("after 1\n");
